...
class GoalAnalyzer:
    def __init__(self, bag_path, n_robots, real=False, quad_name="quadrotor1"):
        ...
        for robot_name in self.robot_list_:
            ...
            self.topic_list_.append(
                f"/{robot_name}/goal_manager/claimed_goals")
    ...
    def analyze(self):
        ...
        for topic, msg, t in tqdm(self.bag_.read_messages(topics=self.topic_list_)):
            ...
            if "pose_est" in topic:
                loc = np.array([msg.pose.pose.position.x,
            ...
            elif topic == f"/{self.quad_name_}/asoom/map" or topic == f"/{self.quad_name_}/asoom/map/compressed":
                ...
                latency = np.float32(str(msg).split(": ")[-1].strip('"'))
        ...
        np.savez(filename, times=np.stack([self.goal_discovery_t_,
                 ...
                 supp=self.goal_claimed_sup_, end_t=end_t-self.start_t_,
        ...
        for goal in goal_msg.points:
            ...
            if self.goal_list_.shape[0] > 0:
                ...
                if min_dist < self.min_goal_dist_m_:
                    # goal already exists
                    self.update_goal_stamps(-1, min_ind, robot_id,
    ...
    def claimed_goal_cb(self, goal_msg, robot_id):
            ...
            if self.goal_list_.shape[0] > 0:
                ...
                if min_dist < self.min_goal_dist_m_:
                    ...
                    self.update_goal_stamps(
                        goal.status, min_ind, robot_id, goal_msg.header.stamp.to_sec() - self.start_t_)
                    continue

            self.add_goal(
                goal_np, goal.status, goal_msg.header.stamp.to_sec() - self.start_t_, robot_id)
        ...
        for g_id in goals_unclaimed:
            if self.goal_unclaimed_t_[robot_id][g_id] == 0:
                self.goal_unclaimed_t_[
                    robot_id][g_id] = goal_msg.header.stamp.to_sec() - self.start_t_
    ...
    def add_goal(self, goal_loc, goal_status, t, robot_id):
        ...
        self.update_goal_stamps(goal_status, len(
            self.goal_list_)-1, robot_id, t)
    ...
    def pose_cb(self, loc, robot_id):
        ...
        if self.last_poses_[robot_id] is not None:
            self.dists_[
                robot_id] += np.linalg.norm(loc - self.last_poses_[robot_id])
...
if __name__ == '__main__':
    ...
    parser.add_argument("-q", "--quad_name",
                        help="quadrotor name(s)", default="quadrotor1")
