...
def plot_goal_reached_times(comp_paths: List[List[str]],
                            time_lim: Optional[float] = np.Inf,
                            existing_plt: Union[Tuple[plt.Figure,
                                                      plt.Axes], None] = None,
    ...
    for paths in comp_paths:
        ...
        for path in paths:
            ...
            # set goals over time limit to nan to keep following logic
            goal_t[goal_t > time_lim] = np.nan
            end_t = time_lim

            num_unreached = np.count_nonzero(np.isnan(goal_t))
            total_num_unreached += num_unreached
            total_num_reached += np.count_nonzero(~np.isnan(goal_t))
            ...
            # TODO doesn't scale to multiple bags
            stats.append({"unreached": num_unreached,
                          "reached": np.count_nonzero(~np.isnan(goal_t)),
                          "times": goal_t})

            optional_keys = ("goal_dists", "robot_dists")
        ...
        diff_num_reached.append(total_num_reached)

    if existing_plt == None:
        fig, ax = plt.subplots()
    else:
        fig, ax = existing_plt
        plot_title = False
    ...
    if plot_title:
        ...
        ax.set_ylabel(
            "Time goal visited (minutes from experiment start)", fontsize=12)
    ...
    if plot_title:
        for ind, (nr, nu) in enumerate(zip(diff_num_reached, diff_num_unreached)):
            ax.text(ind+1, np.max(goal_comp[ind]+0.3),
                    f"{nr}/{nu + nr} goals visited", horizontalalignment='center', fontsize=12)
    return fig, ax, stats
            ...
            for robot_id in d_supp:
                for goal_id in d_supp[robot_id]:
                    ...
                    if d_supp[robot_id][goal_id][-1][0] == "claimed":
                        ...
                        print(
                            f"Something weird is going on here, investigate further. path: {path}, robot: {robot_id}, goal: {goal_map_id}")
    ...
    for dset_ind, paths in enumerate(comp_paths):
        ...
        for p_ind, path in enumerate(paths):
                    ...
                    for goal_id in r_supp:
                        ...
                        for switch in r_supp[goal_id]:
                            if switch[0] == "claimed":
                                ...
                                unvisited_t[goal_id] += min(switch[1],
                                                            end_t) - last_claimed_t

                        if robot_d[3, goal_id] > 0:
                            visited_t[goal_id] += robot_d[3,
                                                          goal_id] - last_claimed_t
            ...
            if all_robot_vis_t is None:
                ...
                all_robot_unvis_t = np.vstack(
                    (all_robot_unvis_t, robot_unvisited_t))
        ...
        x_loc = np.arange(
            all_robot_vis_t.shape[0]) + dset_ind/(len(comp_paths)+1)
        vis_bar = ax.bar(x_loc, all_robot_vis_t, color='blue',
                         width=bar_width, align='edge')
        unvis_bar = ax.bar(x_loc, all_robot_unvis_t, bottom=all_robot_vis_t,
                           color='orange', width=bar_width, align='edge')
        plt.legend((unvis_bar[0], vis_bar[0]),
                   ("Not Visited", "Visited"), fontsize=12)
        plt.ylabel("Fraction of time spent en route to goal", fontsize=12)
        plt.xticks(np.arange(all_robot_vis_t.shape[0])+0.4, [f"Robot {i}" for i in range(
            all_robot_vis_t.shape[0])], rotation='vertical', fontsize=12)
        ax.set_title(
            "Fraction of time spent en route to goal for each robot", fontsize=15)
        ax.tick_params(axis='x', which='both', length=0)
        ax.tick_params(axis='y', labelsize=12)

    fig, ax = plt.subplots()
    ax.set_title(
        "Distribution over fraction of time spent en route to goal", fontsize=15)
    ax.set_ylabel("Fraction of time spent en route to goal", fontsize=12)
    violins = ax.violinplot(dataset=comps, positions=[i for i in range(
        len(comps)//2) for _ in range(2)], showmeans=True)
    ax.set_xticks([])
    ax.set_ylim([0, 1])
    ax.tick_params(axis='y', labelsize=12)
    ...
    for dset_ind, paths in enumerate(comp_paths):
        ...
        for path in paths:
            ...
            if all_robot_goal_num is None:
                ...
                all_robot_goal_num = np.vstack(
                    (all_robot_goal_num, robot_goal_num))

        bar_width = 0.8/(len(comp_paths)+0.2)
        x_loc = np.arange(d.shape[1]) + dset_ind/(len(comp_paths)+1)

        ax.bar(x_loc, np.sum(all_robot_goal_num, axis=0),
               width=bar_width, align='edge')

    plt.xticks(np.arange(
        d.shape[1])+0.4, [f"Robot {i}" for i in range(d.shape[1])], rotation='vertical', fontsize=12)
    ax.tick_params(axis='x', which='both', length=0)
    ax.tick_params(axis='y', labelsize=12)
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        "-d", "--data", help="Path to data to process", nargs='+', action='append')
